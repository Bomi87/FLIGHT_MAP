...
/* ------------------ FR24 FALLBACK SETTINGS ------------------ */

const USE_FR24_FALLBACK = true;

/* 중국 보완 목적: 무료 ADS-B에서 처음부터 안 보이는 경우도 FR24 허용 */
const FR24_ONLY_IF_PREVIOUSLY_SEEN = false;

/* 무료 ADS-B에서 2번 연속 못 잡히면 FR24 시도 */
const FR24_STALE_THRESHOLD = 2;

/* 같은 target에 대한 FR24 최소 재호출 간격 */
...
/* ------------------ FR24 STATE ------------------ */
const lastFr24FetchAtByTarget = new Map();
...
function canCallFr24Now(target) {
  const now = Date.now();
  const last = lastFr24FetchAtByTarget.get(target.key) || 0;
  return (now - last) >= FR24_MIN_INTERVAL_MS;
}

function shouldUseFr24ForTarget(target) {
  const state = aircraftStates.get(target.key);
  if (!state) return false;

  if ((state.staleCount || 0) < FR24_STALE_THRESHOLD) {
    return false;
  }

  if (FR24_ONLY_IF_PREVIOUSLY_SEEN && !state.lastAircraft) {
    return false;
  }

  if (!canCallFr24Now(target)) {
    return false;
  }
...
async function fetchFr24Fallback(targets) {
  if (!USE_FR24_FALLBACK || !Array.isArray(targets) || targets.length === 0) {
    return new Map();
  }
  ...
  try {
    const url = `${FR24_FALLBACK_ENDPOINT}?hexes=${encodeURIComponent(hexes.join(","))}`;
    const data = await fetchJson(url, 5000);

    const aircraftList = Array.isArray(data?.aircraft) ? data.aircraft : [];
    const deduped = dedupeAircraftList(aircraftList);
    const { byHex } = indexAircraftList(deduped);
    ...
    for (const target of targets) {
      const ac = byHex.get(target.hex) || null;
      if (ac) {
        resolved.set(target.key, ac);
        lastFr24FetchAtByTarget.set(target.key, Date.now());
      }
    }

    return resolved;
  } catch (err) {
    console.error("fetchFr24Fallback error:", err);
    return new Map();
  }
}
...
async function fetchBatchAircraftData() {
  ...
  for (const provider of providers) {
    ...
    try {
      const aircraft = dedupeAircraftList(
        await fetchProviderByHex(provider, unresolvedTargets)
      );
  ...
  if (USE_FR24_FALLBACK && unresolvedTargets.length > 0) {
    const fr24Candidates = unresolvedTargets.filter(shouldUseFr24ForTarget);

    if (fr24Candidates.length > 0) {
      const fr24Resolved = await fetchFr24Fallback(fr24Candidates);

      for (const [targetKey, ac] of fr24Resolved.entries()) {
        if (!resolvedMap.has(targetKey)) {
          resolvedMap.set(targetKey, ac);
        }
      }

      unresolvedTargets = unresolvedTargets.filter(t => !fr24Resolved.has(t.key));
    }
  }

  if (resolvedMap.size === 0 && lastError) {
    throw lastError;
  }
